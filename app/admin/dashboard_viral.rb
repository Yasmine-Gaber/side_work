ActiveAdmin.register_page "Dashboard Viral" do

  menu parent: "لوحة التحكم", :priority => 3, :label => "فريق الفايرال"

  content :title => proc{ I18n.t("active_admin.dashboard_viral") } do
    ...
    columns do
      column do
        ...
        ((Date.today-1.month)..Date.today).each do |day|
          x_axis << day.to_s
          viral_list << ViralArticle.approved.created_between(day, day).size
          total_list << Article.approved.created_between(day, day).size
        end
        data_series = [
          {name: "كل الفرق", data: total_list},
          {name: "فريق الآخبار", data: viral_list}
        ]
    ...
    columns do
      column do
        panel 'تقسيم المقالات بالوضع' do
          data_series = []
          data_series << {name: "موافقة", y: ViralArticle.approved.size}
          data_series << {name: "استفسار", y: ViralArticle.inquired.size}
          data_series << {name: "رُفض", y: ViralArticle.rejected.size}
          data_series << {name: "معلق", y: ViralArticle.pending.size}
      ...
      column do
        panel 'تقسيم المقالات بالحالة' do
          data_series = []
          data_series << {name: "نُشر", y: ViralArticle.where(status_id: Status.where(key: "published").first.try(:id)).size}
          data_series << {name: "لم ينشر", y: ViralArticle.where(status_id: Status.where(key: "not_published").first.try(:id)).size}
          data_series << {name: "تم التكليف", y: ViralArticle.where(status_id: Status.where(key: "received").first.try(:id)).size}
          data_series << {name: "تم الإستلام", y: ViralArticle.where(status_id: Status.where(key: "assigned").first.try(:id)).size}
          data_series << {name: "تم الرفض", y: ViralArticle.where(status_id: Status.where(key: "rejected").first.try(:id)).size}
          data_series << {name: "غير محدد", y: ViralArticle.where(status_id: nil).size}
    ...
    columns do
      column do
        panel 'تقسيم المقالات المقترحة بالتصنيف' do
          ...
          Interest.all.each do |interest|
            ...
            daily_counts << ViralArticle.created_daily_by_interest(interest.id).size
            weekly_counts << ViralArticle.created_weekly_by_interest(interest.id).size
            monthly_counts << ViralArticle.created_monthly_by_interest(interest.id).size
          end
          data_series = [
      ...
      column do
        panel 'تقسيم المقالات المقبولة بالتصنيف' do
          ...
          monthly_counts = []
          monthly_targets = []
          approved_articles = ViralArticle.approved
          Interest.all.each do |interest|
            x_axis << interest.title
      ...
      column do
        panel 'احدث المراسلين' do
          table_for Reporter.order('id desc').limit(10) do 
            ...
            column('عدد المقالات') { |reporter| ViralArticle.where(user_id: reporter.id).size }
          end
        end
      end
    end
