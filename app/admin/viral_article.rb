ActiveAdmin.register ViralArticle do
  menu false # parent: 'الفايرال', priority: 11
  actions :all, except: [:destroy]
  # permit_params :line_manager_id, :status_id, :interest_id, :start_date, :deadline, :publish_date, :editor_id, :assignment_id, :drive_link, :cms_link, :notes, :approval_comments, :article_country_id, :original_words_count, persona_ids: []
  ...
  index do
    at = ArticleType.where(key: "viral").first
    ...
    articles_counts = [ViralArticle.created_daily.size, ViralArticle.created_weekly.size, ViralArticle.created_monthly.size]
    render(partial: 'admin/statistics/gauge_statistics_area', locals: {chart_title: "التقدم في عدد المقالات - فايرال", div_ids: div_ids, guage_titles: guage_titles, articles_counts: articles_counts, target_values: target_values})
