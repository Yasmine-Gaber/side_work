ActiveAdmin.register NewsArticle do
  menu false #parent: 'الآخبار', priority: 11
  actions :all, except: [:destroy]
  
  permit_params :title, :team_id, :team, :publish_flag, :article_type, :article_type_id, :line_manager_id, :status_id, :interest_id, :start_date, :deadline, :publish_date, :editor_id, :assignment_id, :drive_link, :cms_link, :notes, :approval_comments, :article_country_id, :original_words_count, :details, :publisher, :publisher_id, :reporter_desk_notes, :trans_desk_notes, :reporter_notes, :manager_notes, persona_ids: []
  ...
  controller do
    ...
    #   Article.where(article_type_id: ArticleType.where(key: "viral").first.try(:id))
    # end
  ...
  index do
    at = ArticleType.where(key: "news").first
    guage_titles = ['هدف يومي', 'هدف اسبوعي', 'هدف شهري']
    div_ids = ["daily_target", "weekly_target", "monthly_target"]
    target_values = [at.daily_target, at.weekly_target, at.monthly_target]
    articles_counts = [NewsArticle.created_daily.size, NewsArticle.created_weekly.size, NewsArticle.created_monthly.size]
    render(partial: 'admin/statistics/gauge_statistics_area', locals: {chart_title: "التقدم في عدد المقالات - الآخبار", div_ids: div_ids, guage_titles: guage_titles, articles_counts: articles_counts, target_values: target_values})
  ...
  form do |f|
    f.inputs do
      f.input :article_type, :input_html => { :disabled => true } 
      f.input :team
      f.input :status
      f.input :title
